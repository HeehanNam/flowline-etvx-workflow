...
export function cloneWorkflowAsTemplate(source, name = source.name) {
  if (!source?.tasks || !Array.isArray(source.tasks)) throw new Error("유효한 Workflow 정의가 아닙니다.");
...
export function parseWorkflow(text) {
  ...
  const source = payload?.format === FORMAT ? payload.workflow : payload;
...
export function buildRunReport(run) {
  ...
  const taskRows = workflow.tasks.map((task, index) => {
    ...
    const checked = runtime?.checklist.filter(item => item.checked).length || 0;
    const subDone = runtime?.subtasks.filter(item => item.completed).length || 0;
    return `<section class="task"><header><span>${String(index + 1).padStart(2, "0")}</span><div><h2>${escapeHtml(task.title)}</h2><p>${escapeHtml(task.assignee)} · ${escapeHtml(task.priority)}</p></div><b class="status ${runtime?.status || "blocked"}">${statusLabels[runtime?.status] || "진입 대기"}</b></header><div class="etvx"><div><b>E</b><p>${escapeHtml(task.entry || "-")}</p></div><div><b>T</b><p>${escapeHtml(task.instructions || "-")}</p></div><div><b>X</b><p>${escapeHtml(task.exit || "-")}</p></div></div>${task.subtasks.length ? `<h3>Subtask <small>${subDone}/${task.subtasks.length}</small></h3><ul>${task.subtasks.map(item => `<li>${runtime?.subtasks.find(value => value.itemId === item.id)?.completed ? "☑" : "☐"} ${escapeHtml(item.title)}${item.required ? " <em>필수</em>" : ""}</li>`).join("")}</ul>` : ""}<h3>Validation 체크리스트 <small>${checked}/${task.checklist.length}</small></h3><ul>${task.checklist.map(item => `<li>${runtime?.checklist.find(value => value.itemId === item.id)?.checked ? "☑" : "☐"} ${escapeHtml(item.text)}${item.required ? " <em>필수</em>" : ""}</li>`).join("")}</ul></section>`;
  }).join("");
