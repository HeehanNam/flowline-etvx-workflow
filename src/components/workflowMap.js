...
export function workflowMap(workflow,instance=null,selected=null,expandedIds=[],zoom=1,selectedConnection=null){
 if(!workflow?.tasks.length)return `<div class="empty"><span>◇</span><strong>아직 Task가 없습니다</strong><p>첫 Task를 추가해 흐름을 만들어 보세요.</p></div>`;
...
function connectionPath(workflow,fromId,toId,selectedConnection){
 ...
 if(!from||!to)return "";
 const selected=selectedConnection?.from===fromId&&selectedConnection?.to===toId;
...
function taskNode(task,workflow,instance,selected,expandedIds,index){
 const run=instance?.tasks.find(item=>item.taskId===task.id),status=run?.status||"definition";
 const deps=task.dependencies.map(id=>workflow.tasks.find(item=>item.id===id)?.title).filter(Boolean);
 const expanded=expandedIds.includes(task.id),subtaskState=id=>run?.subtasks.find(item=>item.itemId===id)?.completed;
